...
func TestSimpleChat(t *testing.T) {
	...
	provider := anthropic.New(log, e.AnthropicKey)
	client := llm.New(log, provider)
	agent := client.Agent(llm.WithModel("claude-3-5-haiku-latest"))
...
func TestConversation(t *testing.T) {
	...
	// First turn
	content1 := collectResponse(t, agent, ctx, "My favorite color is blue. Remember that.")
	is.True(len(content1) > 0)

	// Second turn - should remember context
	content2 := collectResponse(t, agent, ctx, "What is my favorite color?")
	is.True(strings.Contains(strings.ToLower(content2), "blue"))
}
...
func TestAgentClear(t *testing.T) {
	...
	// Set some context
	collectResponse(t, agent, ctx, "The secret word is 'banana'.")

	// Clear the conversation
	agent.Clear()

	// Ask about the secret - should not remember
	content := collectResponse(t, agent, ctx, "What is the secret word I told you?")
	is.True(!strings.Contains(strings.ToLower(content), "banana"))
}
...
func TestToolMultiTurnGathering(t *testing.T) {
	...
	is.True(gatheredName == "Alice")
	is.True(gatheredCity == "Seattle")
	is.True(strings.Contains(content, "Alice"))
	is.True(strings.Contains(content, "Seattle"))
}
