...
func TestSimpleChat(t *testing.T) {
	...
	agent := client.Agent(llm.WithModel("gpt-4o-mini"))
...
func TestConversation(t *testing.T) {
	...
	// First turn
	content1 := collectResponse(t, agent, ctx, "My favorite animal is a penguin. Remember that.")
	is.True(len(content1) > 0)

	// Second turn - should remember context
	content2 := collectResponse(t, agent, ctx, "What is my favorite animal?")
	is.True(strings.Contains(strings.ToLower(content2), "penguin"))
}
...
func TestToolSingleCall(t *testing.T) {
	...
	var content string
	for event, err := range agent.Chat(ctx, "Use the multiply tool to multiply 6 and 7, then tell me the result.") {
		is.NoErr(err)
		if event.Tool != nil {
			sawToolCall = true
			is.Equal(event.Tool.Name, "multiply")
		}
		if event.Done {
			content = event.Content
		}
...
func TestToolMultiTurnGathering(t *testing.T) {
	...
	is.True(gatheredName == "Bob")
	is.True(gatheredCity == "Portland")
	is.True(strings.Contains(content, "Bob"))
	is.True(strings.Contains(content, "Portland"))
}
