...
func loadEnv(t *testing.T) *env.Env {
	...
	if e.OllamaModel == "" {
		e.OllamaModel = "llama3.2:latest"
	}
	return e
}

func newProvider(t *testing.T, e *env.Env) *ollama.Client {
	t.Helper()
	log := logs.Default()
	if e.OllamaHost == "" {
		return ollama.Default(log)
	}
	u, err := url.Parse(e.OllamaHost)
	if err != nil {
		t.Fatalf("ollama: parsing host: %v", err)
	}
	return ollama.New(log, u)
}
...
func skipIfNoOllama(t *testing.T, provider *ollama.Client) {
	t.Helper()
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	_, err := provider.Models(ctx)
	if err != nil {
		t.Fatal("Ollama not available: ", err)
	}
}

func skipIfNoModel(t *testing.T, provider *ollama.Client, model string) {
	t.Helper()
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	...
	t.Skipf("Model %q not available in Ollama", model)
}
...
func TestModels(t *testing.T) {
	...
	provider := newProvider(t, e)
	skipIfNoOllama(t, provider)
...
func TestSimpleChat(t *testing.T) {
	...
	provider := newProvider(t, e)
	skipIfNoModel(t, provider, e.OllamaModel)

	client := llm.New(log, provider)
	agent := client.Agent(llm.WithModel(e.OllamaModel))
...
func TestConversation(t *testing.T) {
	...
	// First turn
	content1 := collectResponse(t, agent, ctx, "My favorite color is green. Remember that.")
	is.True(len(content1) > 0)

	// Second turn - should remember context
	content2 := collectResponse(t, agent, ctx, "What is my favorite color?")
	is.True(strings.Contains(strings.ToLower(content2), "green"))
}
...
func TestToolSingleCall(t *testing.T) {
	...
	client := llm.New(log, provider)
	agent := client.Agent(
		llm.WithModel(e.OllamaModel),
		llm.WithTool(addTool),
	)

	var sawToolCall bool
	var content string
	for event, err := range agent.Chat(ctx, "Use the add tool to add 17 and 25, then tell me the result.") {
		is.NoErr(err)
		if event.Tool != nil {
			sawToolCall = true
			is.Equal(event.Tool.Name, "add")
		}
		if event.Done {
			content = event.Content
		}
...
func TestToolMultiTurnGathering(t *testing.T) {
	...
	gatherNameTool := llm.Func("gather_name", "Ask the user for their name", func(ctx context.Context, in struct {
		...
		gatheredName = "Dave"
		return "User's name is Dave", nil
	})
	...
	gatherCityTool := llm.Func("gather_city", "Ask the user for their city", func(ctx context.Context, in struct {
		...
		gatheredCity = "Austin"
		return "User's city is Austin", nil
	})
	...
	is.True(gatheredName == "Dave")
	is.True(gatheredCity == "Austin")
	is.True(strings.Contains(content, "Dave"))
	is.True(strings.Contains(content, "Austin"))
}
